...
# Information
- In this document the historical glucose values are used to extract features based on CGM data.
- First raw data from each patient needs to be processed in a way that a generalized approach is available for all patients.
- This includes categorizing the data in specific times and also to extract outcome variables like Hypoglycemia or Hyperglycemia
- In the last steps the features need to be extracted which are based on literature and expert knowledge
...
### First data set
A for loop and a list format is used first as there were many structual differences within the raw data in order to avoid to filter wanted patients or select unwanted patients when working in the same dataset.
```{r}
...
cgm_all_extra[[i]] <- cgm_all_extra[[i]] %>%
  mutate(date = dmy(ID),  # this date format is differently
         time = hm(Tijd),
         time_num = as.numeric(time), # Time is saved in seconds, which makes it possible to split all days based on a specific time. This is especially useful as not the full dates can be used for that as patients were monitored for different length and at different times
         year = year(date),
         month = month(date),
         mday = mday(date),
         wday = wday(date, label =TRUE),
         weekend = as.factor(case_when(wday == "Sa" |wday == "So" ~ "Weekend",
                                       wday == "Mo" |wday == "Di" | wday == "Mi" |wday == "Do" |wday == "Fr" ~ "Week")), # these are already features indicating the day of the week, as previous studies showed that this might have an influence on the risk of NH
         hour = hour(time),
         minute = minute(time),
...
### Second data set
Some time formats are not compatible so the approach is repeated for this second group of patients
```{r}
...
cgm_all[[i]] <- cgm_all[[i]] %>%
  mutate(date = ymd(ID), # some time formats are not compatible so the approach is repeated for this second group of patients
         time = hm(Tijd),
...
### Interval creation
Based on the time of the very first date the intervals are constructed in a way to not waste any day, but have them in the right format. This is important in order to always have the same data available.
```{r}
...
for(i in 1:length(cgm_all)){
  if(cgm_all[[i]]$Intervals_before_bedtime[1] == "hour_0_1_before" | cgm_all[[i]]$Intervals_before_bedtime[1] == "hours_1_2_before" | cgm_all[[i]]$Intervals_before_bedtime[1] == "hours_2_3_before" | cgm_all[[i]]$Intervals_before_bedtime[1] == "hours_3_4_before" | cgm_all[[i]]$Intervals_before_bedtime[1] == "hours_4_5_before" | cgm_all[[i]]$Intervals_before_bedtime[1] == "hours_5_6_before"){
    ...
    intervals <- as.numeric(seq(ymd_hms(cgm_all[[i]]$full_date[1]), by = "24 hours", length.out = length(unique(cgm_all[[i]]$date)))) # based on the time of the very first date the intervals are constructed in a way to not waste any day, but have them in the right format. This is important in order to always have the same data available. I.e., if a patient started tracking at 6pm the intervals before that needed for prediction are not available for that day. In this way, all of the days are seperated and brought in the same format.
...
### Exclusion criterion
```{r}
# delete periods with less multiple missing observations per interval to have an estimate for each category
cgm_all_count <- list()
for(i in 1:length(cgm_all)){
...
- Exclusion of night time to only calculate indices based on day time - otherwise we would use information we wouldn't have for the normal prediction (Last before bed - LBB)
```{r}
...
rename(id = patient, "gl_mmol/l" = Historie_glucose, time_old = time, time = full_date) %>% # for the data to be processed  by the iglu package they need to have specific variable names, which is done here accordingly
...
## Indices
```{r}
# Multiple indices are extracted from the original data set and calculated already for each day individually, as this is the information we want to vary for each day.
hbgi <- list()
for(i in 1:length(iglu_periods)){
...
```{r}
# Bind all of the indices and add the corresponding period to make it possible to identify which indices are conducted for which period
...
indic_per <- indic_per %>%
  rename(patient = id) # This can later be included in the full data set as information about the period and about the patient id is given, which is enough to join them.
```

## Features per time interval
Feature extraction for each time interval for descritpive statistics like mean, sd, min, max
```{r}
...
for(i in 1:length(cgm_all)){
  cgm_all[[i]] <- cgm_all[[i]] %>%
  group_by(periods) %>%
     mutate(Night_class_hyper_30 = ifelse(sum(Night_class_hyper == 1 & Nocturnal == "Night") >= sum(Night_class_hyper == 0 & Nocturnal == "Night")*0.3, 1,0)) %>% # a second classification for Nocutrnal Hypreglycaemia which is suggested by the ADA
    ungroup()
}
```

# Working with a single data set
This data set contains the historical data for each patient individually. On top of that already some distinctions and features are caluclated.
```{r}
# all days per person with valid features
...
## Feature extraction - Per interval

## Feature extraction - Slopes
First also features are extracted based on the slopes of historical glucose values and the increasing time. This suggests trends over time and if they are decreasing or increasing in a specific interval. These features have been used in multiple studies for NH prediction already.
```{r, warning=FALSE}
...
Here it is grouped by the interval to have estimates for each interval
```{r}
...
Here only estimates for either day or night are conducted.
```{r}
# Feature extraction to have the features for each observation separately
cgm_per_day_noc <- all_patients_cgm %>%
  ...
  ungroup()
```
These are two additional features providing information about how many Hypo/Hyper events occured on a certain day (Day/Night)
```{r}
...
Now the data still needs to be in a format to make it easier to analyse. Therefore some of the rows are pivoted to be columns. This is done for all features extracted for specific time intervals. I.e., Mean for 1 hour before bedtime, Mean for 1 to 2 hours before bedtime, etc. as columns

First, day night with night further seperated in early and late night
```{r}
...
Same for day and night division
```{r}
...
From here onwards all the data sets which were created are merged into a single data set including all CGM features
```{r}
...
From here onwards data are checked
```{r}
...
```
One of the further exclusion criteria is that at least 5 days of full CGM measurements need to be conducted. Like this some patients with less are identified and excluded.
```{r}
...
### Save data sets
Data is saved for further analyses and to be joined by other data formats like physical acticvity and food entries.
```{r}
