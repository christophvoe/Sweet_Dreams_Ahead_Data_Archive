...
A Threshold is applied with 0.01
```{r}
options(scipen = 999)
...
Due to lower similarity with non CGM features a lower treshold is tested for only the variables in the Lifestyle condition. Values stay the same to the approach before as the mutual information approach is independent of any model and always produces the same estimates no matter how many variables are included.
```{r}
options(scipen = 999)
...
colnames(full_data_sel)
colnames(full_data_life_plus_sel)
```
